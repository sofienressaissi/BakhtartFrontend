...
export default function SeenRecently() {
    ...
    const [seenProds, setSeenProds] = useState([]);
    let [bakhtartProds, setBakhtartProds] = useState([]);
    let [bakhtCats, setBakhtCats] = useState([]);
    ...
    useEffect(async() => {
      ...
      seenProds.map((itemsp,index)=>{
        userrData.userr && 
        userrData.userr.id === itemsp.userId ? nbSeen = nbSeen + 1 : <></>
      });

      const currentTableData = useMemo(() => {
        const firstPageIndex = (currentPage - 1) * PageSize;
        const lastPageIndex = firstPageIndex + PageSize;
        return seenProds.slice(firstPageIndex, lastPageIndex);
      }, [currentPage, seenProds]);
          ...
          for (let j = 0; j < bakhtartProds.length; j++) {
              if (currentTableData[i].productId === bakhtartProds[j]._id
                && userrData.userr && userrData.userr.id === currentTableData[i].userId) {
                  seenByDate.push(bakhtartProds[j]);
              }
          }
      }
    ...
    return(
        <>
        {
            ...
            <html>
            <head>
                <title>Bakht'Art - Seen Recently</title>
                        ...
                        <div className="profile-head">
                            ...
                            <ul className="nav nav-tabs" id="myTab" role="tablist">
                                ...
                                <li className="nav-item">
                                    ...
                                    aria-selected="false"
                                    style={{color: '#021144', fontFamily: 'Felix Titling'}}>Seen Recently</a>
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div className="row">
                    <div className="col-md-4">
                        <div className="profile-work">
                            ...
                            <a href="/my-orders" 
                            style = {{color:'#212529', fontFamily: 'Felix Titling'}}>My Orders</a><br/>
                            <a href="/my-wishlist" 
                            style = {{color:'#212529', fontFamily: 'Felix Titling'}}>My Wishlist</a><br/>
                            <a href="/seen-recently" 
                            style = {{color:'#021144', fontFamily: 'Felix Titling'}}>Seen Recently</a><br/>
                            <p></p>
                        </div>
                    </div>
    ...
    font-size: 9px;\
      ...
      `}
                                   </style>
                                   {
                                       ...
                                       <div align = "center">
                                       <span style={{fontFamily: 'Felix Titling',
                                    color: '#070C29', fontWeight: 'bold'}}>No product has been recently seen</span>
                                       </div> : <></>
                                   }
                                   {
                                       nbSeen > 0 ?
                       ...
                       style={{marginRight: '250px;'}}
                        placeholder="Search Seen Products"
                   onChange={(e)=>setSearchitem(e.target.value)}/><br/><br/>
                                       </> : <></>
                                   }
                                   
                                   <table cellspacing="0" className="shop_table cart" width="30px">
                                        <thead>
                                            {
                                                nbSeen > 0 ?
                                                <>
                                            ...
                                            style={{fontFamily: 'Felix Titling', color: '#021144'}}><h5><b>Description</b></h5></th>
                                            <th className="product-thumbnail"
                                            style={{fontFamily: 'Felix Titling', color: '#021144'}}><h5><b>Price</b></h5></th>
                                            <th className="product-thumbnail"
                                            style={{fontFamily: 'Felix Titling', color: '#021144'}}><h5><b>Category</b></h5></th>
                                            <th className="product-thumbnail"
                                            style={{fontFamily: 'Felix Titling', color: '#021144'}}><h5><b>Availability</b></h5></th>
                                        </tr>
                                                </> : <></>
                                            }
                                        </thead>
                                        <tbody>
                                            {
                                                seenByDate.filter((item=>{
                                                    ...
                                                    currentTableData.map((itemps => (
                                                        itemp._id === itemps.productId
                                                        && userrData.userr
                                                        && userrData.userr.id === itemps.userId ?
                                                        <>
                                                        <tr className="cart_item" key = {itemps.productId}>
                                                            ...
                                                            <td className="product-thumbnail">
                                                                <span style={{fontFamily: 'Felix Titling', 
                                                                color: '#070C29'}}>{itemp.productPrice} TND</span>
                                                            </td>
                                                            <td className="product-thumbnail">
                                                                {
                                                                    bakhtCats.map((itembc => (
                                                                        itembc.categoryPath === itemp.productCategory ?
                                                                        <>
                                                                        <span style={{fontFamily: 'Felix Titling', 
                                                                color: '#070C29'}}>{itembc.categoryName}</span>
                                                                        </> : <></>
                                                                    )))
                                                                }
                                                            </td>
                                                            <td className="product-thumbnail">
                                                                {
                                                                    ...
                                                                    <>
                                                                    <span style={{fontFamily: 'Felix Titling', 
                                                                color: 'red', fontWeight: 'bold'}}>Out Of Stock</span>
                                                                    </> : <>
                                                                    <span style={{fontFamily: 'Felix Titling', 
                                                                color: 'green', fontWeight: 'bold'}}>In Stock<br/><small>({itemp.productQuantity} items)</small></span>
                                                                    </>
                                                                }
